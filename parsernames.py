...
class MoodleParser:
  ...
  def strip_accents(self,string,encoding):
    return unicodedata.normalize('NFKD', unicode(string, encoding)).encode('ASCII', 'ignore')

  def cleanupName(self,name,encoding):
    name = self.strip_accents(name,encoding)
    name = name.title()
    return name.replace(' ','')

  def generatefiles(self,inputfile,encoding,urlmoodle,outputfile):
    ...
    with open(outputfile,'w') as fw:
      ...
      for value in values:
        name = self.cleanupName(value[1],encoding)
  ...
  def run(self):
    ...
    self.generatefiles(options.inputfile,options.encoding,options.url,options.outputfile)
